...
func TestRawTCPDialer(t *testing.T) {
...
func TestRawDialerAESTCP(t *testing.T) {
	key := make([]byte, 16)
	rand.Read(key)
	l1, err := corenet.CreateListenerAESTCPPortAdapter(0, key)
	if err != nil {
		t.Fatal(err)
	}
	listener := corenet.NewMultiListener(l1)
...
func TestRawDialerUDP(t *testing.T) {
	l1, err := corenet.CreateListenerUDPPortAdapter(0)
	if err != nil {
		t.Fatal(err)
	}
	listener := corenet.NewMultiListener(l1)
